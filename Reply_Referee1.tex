...
As now discussed in the text, we believe that this procedure is promising because of its ``automatic'' nature. It is potentially easier to implement to cases
of interest to applied researchers for which the analytical correction has not been developed, for example, ``to construct accurate confidence intervals in settings where unusual forms of dependence would make it difficult to derive CCT's analytical correction.'' Surely, we do not achieve this level of generality in this paper but we hope it will serve as a stepping stone for these developments.
