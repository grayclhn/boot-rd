...
comparing treated and untreated units at the cutoff. Since being just above
or below the arbitrary cutoff is credibly not related to other factors impacting
outcomes, including their unobserved characteristics,  differences between 
treated and untreated individuals at the cutoff can be plausibly attributed to the
...
and~\eqref{eq:6}.\footnote{%
  One subtle but important feature of $\Bf_{-}$ and $\Bf_{+}$ is that they only
  depend on the running variables, not the outcomes $Y_1,\dots,Y_n$.} %
The plug-in bias-corrected estimator then requires estimates
