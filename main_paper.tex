...
Let $h_n$ be the bandwidth used to estimate the local polynomial of order $p$ and let $b_n$ be the bandwidth used for $p+1$. Then $h_n$, $b_n = o(n^{-1/5})$ and
$n h_n$, $n b_n \to \infty$ as $n \to \infty$.\footnote{Unless otherwise stated, all limits in this paper are assumed to hold as $n \to \infty$.}
\end{assumption}
...
\begin{algorithm}[Bias estimation]\label{Alg1}
  ...
  \item Repeat the following steps $B_1$ times to produce a sequence of bootstrap
    ...
    \item Calculate $\hat\mu_+^*$ and $\hat\mu_-^*$ as
      \begin{align}
        ...
        &= \argmin_{\beta_0} \min_{\beta_1,\dots,\beta_p}
          \sum_{i = 1}^{M^-(h_n)} (Y_i^{*-} -
          (\beta_0 + \beta_1 X_i^{*-} + \cdots + \beta X_i^{*-}))^2 \\
        \hat\mu_+^*
        &= \argmin_{\beta_0} \min_{\beta_1,\dots,\beta_p}
          \sum_{i = 1}^{M^+(h_n)} (Y_i^{*+} -
          (\beta_0 + \beta_1 X_i^{*+} + \cdots + \beta X_i^{*+}))^2
    ...
    \end{enumerate}
  \item Calculate the bias estimator as
    \begin{equation}
      \label{eq:4}
      \hat\Delta = \tfrac{1}{B_1} \sum_{k=1}^{B_1} \hat\tau^*_k -
...
\begin{lemma}\label{L1}
  Let $\hat\Delta^*$ be the bias estimator defined in Equation~\eqref{eq:4}.
  Under Assumptions~\ref{A1} and~\ref{A3},
  \[
    \hat\Delta - \big[\E(\hat\tau) - \tau\big] = o_p(-).
  \]
\end{lemma}
...
\begin{algorithm}[Confidence intervals]\label{Alg2}
  ...
  \item Repeat the following steps $B_2$ times to produce a sequence of bootstrap
    ...
    \item Apply Algorithm~\ref{Alg1} to the bootstrapped data set,
      ...
      bootstrap data. Let $\hat\Delta^*$ represent the bias estimate returned
      by Algorithm~\ref{Alg1}.
    \item Save the bias-corrected estimate
      $\hat\mu_+^* - \hat\mu_i^* - \hat\Delta^*$ as $\hat\tau_k^{\prime*}$.
    \end{enumerate}
...
Under the same assumptions as earlier, this procedure produces asymptotically
valid confidence intervals.
\begin{theorem}
  Suppose Assumptions~\ref{A1} and~\ref{A3} hold and let $\hat \ell(\alpha)$ and
  $\hat u(\alpha)$ be the interval endpoints produced by Algorithm~\ref{Alg2}
  for some $\alpha \in (0,1)$. Also let $\Pr_\tau$ represent the probability
  distribution of the random variables under the assumption that $\tau$ is
  the true value of the ATE. Then
  \begin{equation}
    \label{eq:5}
    \plim_{n \to \infty} \sup_\tau
    \Pr_\tau[\hat\ell(\alpha) \leq \tau \leq \hat u(\alpha)] \geq 1 - \alpha.
  \end{equation}
  Each of the one-sided intervals is asymptotically valid as well:
  \begin{align}
    \plim_{n \to \infty} \sup_\tau
