...
these terms are given in the Mathematical Appendix as Equations~\eqref{eq:3}
and~\eqref{eq:4}. The plug-in bias-corrected estimator then requires estimates
    ...
    \item Calculate $\hat\mu_+^*(h)$ and $\hat\mu_-^*(h)$ by estimating the
      ...
      \begin{align*}
        ...
        &= \argmin_{\mu} \min_{\beta} \ssum[+]{i}{h}
          (Y_i^* - \mu - \beta X_i^*)^2.
      \end{align*}
    \item Save $\hat\mu_+^*(h) - \hat\mu_-^*(h)$ as $\hat\tau^*_k$.
    \end{enumerate}
...
\begin{algorithm}[Confidence intervals]\label{Alg2}
  ...
  \item Repeat the following steps $B_{2}$ times to produce a sequence of bootstrap
    ...
    \begin{enumerate}
    \item Draw an i.i.d. sample of size $M_-(b)$ from
      $\{\hat\varepsilon_{i} : i \in I_-(b)\}$ and one of size $M_+(b)$ from
      $\{\hat\varepsilon_i : i \in I_+(h)\}$. Let $\varepsilon_{i}^{*-}$ and
      $\varepsilon_i^{*+}$ denote the $i$th element of each sample and construct
      \begin{align*}
        Y_{-,i}^* &= \hat g_{p+1}^-(X_i^{*-}) + \varepsilon_i^{*-} &
        Y_{+,i}^* &= \hat g_{p+1}^+(X_i^{*+}) + \varepsilon_i^{*+}
      \end{align*}
    ...
    \item Apply Algorithm~\ref{Alg1} to the bootstrapped data set,
      \begin{equation*}
        (Y_1^{*-}, X_1^{*-}),\dots,(Y_{M_-(b)}^{*-},X_{M_-(b)}^{*-}),
        (Y_1^{*+}, X_1^{*+}),\dots,(Y_{M_+(b)}^{*+},X_{M_+(b)}^{*+}),
      \end{equation*}
      using the same bandwidths $h$ and $b$ that are used in the rest of
...
Repeat the steps from Theorem~\ref{T1}'s proof through~\eqref{eq:5} to get
\begin{align*}
  \hat\tau(h)^* - &\Delta_h^{**} - \tau^*
  = (\hat\tau^*(h) - \E^* \hat \tau^*(h)) +
  (\E^* \hat \tau^*(h) - \tau^*) - (\E^{**} \hat \tau^{**}_h - \tau^{**}) \\
  &= \hat\tau^*(h) - \E^*\hat\tau^*(h)
   + h^2 (\hat\mu_-^{*(2)}(b) - \mu_-^{*(2)}) \Bf_{-}(h) /2
   - h^2 (\hat\mu_+^{*(2)}(b) - \mu_+^{*(2)}) \Bf_{+}(h) / 2 \\
  &= \Omega^{+}(b)' \epb_{+}^{*} - \Omega^{-}(b)' \epb_{-}^{*},
\end{align*}
where
$\epb_{+}^{*} =
(\varepsilon_{+,1}^{*},\dots,\varepsilon_{+,M_b^{+}}^{*})'$ and
$\Omega^{+}_b$ is an $M_b^{+}$-dimensional vector with $i$th element
$\Omega_{1i}^{+}(b) - \Omega_{2i}^{+}(b)$, which are defined as
\begin{align*}
  \Omega_{1i}^{+}(b) &=
  (1\quad 0)
  \Big(\ssum{j}{h} r_1(X_j / h) r_1(X_j / h)' \Big)^{-1}
    r_1(X_{m^+_i} / h) \1\{h > X_{m_i} \geq 0\}
\intertext{and}
  \Omega_{2i}^{+}(b) &= (0\quad 0 \quad h^2)
...
$\epb_{-}^{*}$ and $\Omega^-(b)$ have the same definitions as
$\epb_{+}^{*}$ and $\Omega^+(b)$ after making the obvious
substitutions of ``$-$'' for ``$+$.''

Given~\eqref{eq:5}, it suffices to prove that
\begin{gather}
  \label{eq:6}
  \rho(V^{-1/2}(h) \Omega^{+}(b)'\epb_{+}^{*},
    V^{-1/2}(h)\Omega^{+}(b)'\epb_{+}) \to^p 0
  \intertext{and}
  \label{eq:7}
  \rho(V^{-1/2}(h)\Omega^{-}(b)'\epb_{-}^{*},
    V^{-1/2}(h)\Omega^{-}(b)'\epb_{-}) \to^p 0
...
We will only prove~\eqref{eq:6} since the proof of~\eqref{eq:7} is
identical. By Theorem 2.1 of \cite{freedman1981}, we have
\begin{align*}
  \rho(V^{-1/2}(h) \Omega^{+}(b)'\epb_{+}^{*}, V^{-1/2}(h)&\Omega^{+}(b)'\epb_{+}) \\
  &\leq \rho(\varepsilon_{+,1}^{*}, \varepsilon_{+,1}) \times
    \tr(V^{-1/2}(h) \Omega^{+}(b)' \Omega^{+}(b)V^{-1/2}(h)) \\
...
$\bar{\varepsilon} = (1/M_b^{+}) \osum{i}{b} \dot{\varepsilon}_i$. Then,
