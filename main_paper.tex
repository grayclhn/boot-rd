...
presents the assumptions that will be used for our theoretical anaysis in
Section~\ref{boot}.
...
regression.\footnote{%
  ...
  bootstrap algorithm proposed in this paper can be extended to accomodate
  higher order polynomials discontinuities in the derivatives of the conditional
  expectation, like ``Kink RD'' design. We focus on the linear case for ease in
  exposition.} %
For this model, if $h$ represents a bandwidth parameter, the estimator
of $\tau$, $\hat\tau(h)$, is
\begin{equation}
  \label{eq:13}
