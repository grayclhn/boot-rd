...
\begin{algorithm}[Bias estimation]\label{Alg1}
  ...
  \begin{align*}
    ...
    I_+(h) &= \{i : 0 \leq X_{i} < h\}.
  \end{align*}
  Also define $M_-(h)$ and $M_+(h)$ to be the number of elements in
  ...
  \item Repeat the following steps $B_1$ times to produce a sequence of
    ...
    $k$th step:
    \begin{enumerate}
    \item Draw an i.i.d. sample of size $M^-(b)$ from
      $\{\hat\varepsilon_i : i \in I_-(b)\}$ and one of size $M_+(b)$ from
      $\{\hat\varepsilon_i : i \in I_+(h)\}$. Let $\varepsilon_i^{*-}$ and
...
\begin{align*}
  \frac{(\hat\tau(h,b) - \Delta^{*}(h,b) - \tau)}{ V^{1/2}(h, b)} \to^{d} N(0,1)
\end{align*}
\end{theorem}
...
\begin{equation*}
    \E^* \hat\tau^{*}_h - \tau^{*} = h^2\, \mu_+^{*(2)} \Bf(h)/2
      - h^2\, \mu_{-}^{*(2)} \Bf_{-}(h)/2.
\end{equation*}
...
Repeat the steps from Theorem~\ref{T1}'s proof through~\eqref{eq:2} to get
\begin{align*}
  \hat\tau_h^* - &\Delta_h^{**} - \tau^*
...
Given~\eqref{eq:2}, it suffices to prove that
\begin{gather}
  \label{eq:3}
...
We will only prove~\eqref{eq:3} since the proof of~\eqref{eq:4} is
