...
\begin{algorithm}[Bias estimation]\label{Alg1}
  ...
  \item Estimate the local polynomials $g_{p+1}^+$ and $g_{p+1}^-$ as defined by
    ...
    \begin{equation}
      ...
      \end{cases}
    \end{equation}
  \item Repeat the following steps $B$ times to produce a sequence of bootstrap
    estimates $\hat\tau_1^*,\dots,\hat\tau_B^*$. For the $k$th step:
    \begin{enumerate}
  ...
  \item Calculate the bias estimator as
    \begin{equation}
      \label{eq:4}
      \hat\Delta = \tfrac{1}{B} \sum_{k=1}^B \hat\tau^*_k -
...
\begin{algorithm}[Confidence intervals]\hfill
 \begin{enumerate}
	...
	\item Apply Algorithm~\ref{Alg1} (starting at step 2) to bootstrap data generated in step 3 and obtain bias corrected estimate $\hat{\tau}^{bc*}$.
